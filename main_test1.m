% Test script approximating a simple function y = f(x) with a Neural Net
...
%
% Test approximating a simple function y = f(x) with a NN. Here y is a
% scalar and x=(x1,x2) is a 2 x 1 vector. We build grids for x1 and x2 and
...
% correct values. 
%
% In this example, you get a fit which is visually a bit off with 5 hidden
% nodes, and start getting a visually good fit around 10 nodes, but with
% the kind not being quite sharp enough. By 15 nodes the kink gets visibly
% sharper, and the mean approx error and R2 are very good. The
% approximation is good and the procedure appears to be reasonably fast.
...
%scalar. Function f(x) accepts input of size 2 x Ns where each of the Ns
%columns is one x value at which to evaluate the function
f = @(x) x(1,:).^2 + 2*x(2,:).^3 + 3*x(1,:).*x(2,:) + 10*(x(1,:)>x(2,:)).*(x(1,:)-x(2,:));
...
Ny = size(y,1); %size of y output (1 x 1 scalar)
...
ys = reshape(y',[N1,N2]);
yhats = reshape(yhat',[N1,N2]);

MM = 2; NN = 3;

figsize = [100,100,1000,700];
